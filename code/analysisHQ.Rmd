...
turnout_statewide <- read_excel("data/Benchmarks/VAP_Turnout_Rates.xlsx", sheet="Statewide")
```
...
#'
#' For each year in a list of CES data entries, this function scans all
#' `*_PARTY_rc` and `*_CANDIDATE_rc` columns and replaces any cell whose
#' value is the name of another `_post` column with the actual `_post`
#' value from the same row.  Values `"Democrat"`, `"Republican"`, `"Other"`,
#' and `NA` are left unchanged.  Candidate columns in years 2006 and 2008
...
#'
#' @details  
#' - Progress is displayed via a console progress bar (`txtProgressBar`).  
...
recode_party_and_candidate_votes <- function(list_all_years) {
  ...
  for (i in seq_along(list_all_years)) {
    ...
    candidate_cols     <- candidate_cols[!grepl("SENATOR2|SENATOREARLY", candidate_cols)]
...
There are 2 ways to compute error in candidate choice: at the party level and at the candidate level. Both are done using the modal category, so actual unit (candidate or party) that received the most votes in the actual election returns. 
```{r}
