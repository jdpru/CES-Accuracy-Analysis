...
source("utilities/recoding_utilities.R")
source("utilities/data_structures.R")
source("utilities/election_return_utilities.R")
```
...
The raw election return files are loaded in the "election validation" section. The following code creates a series of "prop" (proportion) tables for each election, which says what share of votes each candidate received in each election. 
```{r}
# US House
...
# Election Accuracies
Compute election accuracies in each state/race by comparing vote proportions for each major party in the CES to vote proportions in the true election returns
```{r}
...
This will be used to prettify the accuracy tables at the end and split one big accuracy table into a list of yearly accuracy tables
```{r}
make_tables_pretty <- function(df) {
  # 1. split into a list of data‑frames, one per Year
  tables_by_year <- split(df, df$Year)
  
  # 2. apply rounding + renaming to each element
    ...
    tab %>%
      ...
      mutate(across(
        c(
          ...
          Diff_Unweighted_CES_Actual,
          Diff_Weighted_CES_Actual,
          Diff_ANESRake_Actual
        ),
        ~ round(.x, 1)
      )) %>%
      # rename for prettiness
      rename(
        "CES Candidate"         = CES_Candidate,
        "True Candidate"        = True_Candidate,
        "CES Unweighted %"      = CES_Unweighted_Percent,
        "CES Weighted %"        = CES_Weighted_Percent,
        "ANESRake Weighted %"   = ANESRake_Weighted_Percent,
        "Unweighted CES - Actual %" = Diff_Unweighted_CES_Actual,
        "Weighted CES - Actual %"   = Diff_Weighted_CES_Actual,
        "ANESRake - Actual %"       = Diff_ANESRake_Actual
      ) %>%
      
      mutate(
        #Race          = capitalize_each_word(Race),
        State         = toupper(State)            # if you want all-caps states
        #`CES Candidate`  = capitalize_each_word(`CES Candidate`),
        #`True Candidate` = capitalize_each_word(`True Candidate`),
        #CES_Party     = capitalize_each_word(CES_Party),
        #True_Party    = capitalize_each_word(True_Party)
      )
  })
...
create_election_accuracy_tbl <- function(yearly_ces_wt_vars,
  ...
  for (year_data in year_data_list) {
    ...
    for (race_col in available_races) {
      ...
      if (current_year == 2020 & race_col == "HOUSE_rc") {
        ces_df <- ces_df %>%
      ...
      for (state in unique(na.omit(ces_df$POST_STATE_rc))) {
        ...
        # if (state == "NEW HAMPSHIRE" & race_col == "GOV_rc") {
        #   browser()
        # }
...
Create accuracy table, then clean it up and split it into a list of yearly accuracy tables with make_tables_pretty
```{r}
accuracy_table <- create_election_accuracy_tbl(
  yearly_ces_wt_vars,
...
accuracy_tables_pretty <- make_tables_pretty(accuracy_table)
```

# Results
Now that accuracy_tables_pretty is created, compute different accuracy metrics in different vote ranges for each major party.
```{r}
create_overall_summary_table <- function(data_list) {
#' @description
#' Takes a named list of accuracy tables (one data frame per year), and for each combination of year, race, and party,
#' computes the average absolute error and RMSE for three weighting methods (CES‑Provided Weights, CES‑Unweighted, ANESRake)
#' across different vote‑share ranges. These vote-share ranges correspond to different levels of race competetiveness. For example, 
#' if the vote shares were between 45 and 55%, this would suggest a close race, while a vote share of 80% - 100% is a blowout.

  #browser()
  # Initialize an empty data frame for the results
  result <- data.frame(Year = integer(), Party = character(), `Weighting Method` = character(), Metric = character(), Value = numeric(), Race = character(), Num_States = integer(), stringsAsFactors = FALSE)
  
  # Define the ranges for filtering candidates
  ranges <- list(
    "0-100%" = c(0, 100),
    "30-70%" = c(30, 70),
    "40-60%" = c(40, 60),
  ...
  # Loop over each year in the data list
  for (year_name in names(data_list)) {
    year_data <- data_list[[year_name]]
    
    # Loop over each race
    for (race in unique(year_data$Race)) {
      race_data <- subset(year_data, Race == race)
      
      # Loop over each party
      for (party in c("Democrat", "Republican")) {
        # Filter data for the current party
         party_data <- subset(race_data, grepl(party, True_Party, fixed = TRUE))
         
        # Loop over each range
        for (range_name in names(ranges)) {
          range <- ranges[[range_name]]
 
          # Filter candidates based on the vote share range
          filtered_data <- subset(party_data, Actual_Percent >= range[1] & Actual_Percent <= range[2])
          
          # Count the number of unique states
          num_states <- length(unique(filtered_data$State))
          
          # count number of states with more than one row
          num_states_more_than_one <- length(unique(filtered_data$State[duplicated(filtered_data$State)]))
          if (num_states_more_than_one > 0) {
            stop(sprintf(
              "✖ Data error for %s – %s: %d state(s) have more than one row. Aborting.",
              current_year, race, num_states_more_than_one
            ))
          }

          # Calculate metrics for CES Weighted
          ces_avg_abs_error <- mean(abs(filtered_data$`Weighted CES - Actual %`), na.rm = TRUE)
          ces_rmse <- sqrt(mean(filtered_data$`Weighted CES - Actual %`^2, na.rm = TRUE))
          
          # Calculate metrics for CES Unweighted
          ces_unweighted_avg_abs_error <- mean(abs(filtered_data$`Unweighted CES - Actual %`), na.rm = TRUE)
          ces_unweighted_rmse <- sqrt(mean(filtered_data$`Unweighted CES - Actual %`^2, na.rm = TRUE))
          
          # Calculate metrics for ANESRake
          anes_avg_abs_error <- mean(abs(filtered_data$`ANESRake - Actual %`), na.rm = TRUE)
          anes_rmse <- sqrt(mean(filtered_data$`ANESRake - Actual %`^2, na.rm = TRUE))
          
          # Append the results to the result data frame
          result <- rbind(result, 
                          data.frame(Year = year_name, Party = party, `Weighting Method` = "CES-Provided Weights", Metric = paste0("Avg_Abs_Error_", range_name), Value = ces_avg_abs_error, Race = race, Num_States = num_states),
                          data.frame(Year = year_name, Party = party, `Weighting Method` = "CES-Provided Weights", Metric = paste0("RMSE_", range_name), Value = ces_rmse, Race = race, Num_States = num_states),
                          data.frame(Year = year_name, Party = party, `Weighting Method` = "CES-Unweighted", Metric = paste0("Avg_Abs_Error_", range_name), Value = ces_unweighted_avg_abs_error, Race = race, Num_States = num_states),
                          data.frame(Year = year_name, Party = party, `Weighting Method` = "CES-Unweighted", Metric = paste0("RMSE_", range_name), Value = ces_unweighted_rmse, Race = race, Num_States = num_states),
                          data.frame(Year = year_name, Party = party, `Weighting Method` = "ANESRake", Metric = paste0("Avg_Abs_Error_", range_name), Value = anes_avg_abs_error, Race = race, Num_States = num_states),
                          data.frame(Year = year_name, Party = party, `Weighting Method` = "ANESRake", Metric = paste0("RMSE_", range_name), Value = anes_rmse, Race = race, Num_States = num_states))
        }
      }
    }
  }
...
For checking if race accuracies differ based on whether race was federal or state level 
```{r}
federal_vs_state_summary_table <- function(data_list) {
  # Initialize an empty data frame for the results
  result <- data.frame(Year = integer(), Party = character(), Method = character(), Metric = character(), Value = numeric(), Race = character(), Level = character(), stringsAsFactors = FALSE)
  ...
  # Define Federal and State-Level elections
  federal_races <- c("President", "US Senate", "US House")
  state_level_races <- c("Governor", "Attorney General", "Secretary of State", "State Senator", "State Representative")
  ...
  for (year_name in names(data_list)) {
    ...
    for (race in unique(year_data$Race)) {
      ...
      # Determine if the race is Federal or State-Level
      race_level <- ifelse(race %in% federal_races, "Federal", ifelse(race %in% state_level_races, "State-Level", "Unknown"))
      
      # Loop over each party
      for (party in c("Democrat", "Republican")) {
        # Filter data for the current party
        party_data <- subset(race_data, True_Party == party)
        ...
        for (range_name in names(ranges)) {
          ...
          # Calculate metrics for CES
          ces_avg_abs_error <- mean(abs(filtered_data$`Weighted CES - Actual %`), na.rm = TRUE)
          ces_rmse <- sqrt(mean(filtered_data$`CES - Actual %`^2, na.rm = TRUE))
          ...
          # Append the results to the result data frame
          result <- rbind(result, 
                          data.frame(Year = year_name, Party = party, Method = "CES", Metric = paste0("Avg_Abs_Error_", range_name), Value = ces_avg_abs_error, Race = race, Level = race_level),
                          data.frame(Year = year_name, Party = party, Method = "CES", Metric = paste0("RMSE_", range_name), Value = ces_rmse, Race = race, Level = race_level),
                          data.frame(Year = year_name, Party = party, Method = "ANESRake", Metric = paste0("Avg_Abs_Error_", range_name), Value = anes_avg_abs_error, Race = race, Level = race_level),
                          data.frame(Year = year_name, Party = party, Method = "ANESRake", Metric = paste0("RMSE_", range_name), Value = anes_rmse, Race = race, Level = race_level))
        }
      }
    }
  }
...
Create final list of accuracy tables. 
partitioned_accuracy_results serves as the input to the figuresHQ notebook used for creating figures.
```{r}
partitioned_accuracy_results <- create_overall_summary_table(accuracy_tables_pretty)
write_xlsx(partitioned_accuracy_results, "tables_and_figures/partitioned_accuracy_results.xlsx")
federal_vs_state_hist_table <- federal_vs_state_summary_table(accuracy_tables_pretty)
```


# Table creation
Number of States and Average Number of State Respondents by Office and Year
```{r}
col_order <- c(
  "President",
  "U.S. Senate",
...
# 1) avg respondents
avg_respondents_table <- accuracy_table %>%
  mutate(Year = as.numeric(Year)) %>%
  distinct(Year, Race, State, n_respondents) %>%
  group_by(Year, Race) %>%
  summarise(Average_n_respondents = round(mean(n_respondents)), .groups = "drop") %>%
  pivot_wider(
    names_from  = Race,
    values_from = Average_n_respondents
  ) %>%
  rename(
    President             = President,
  ...
  select(Year, all_of(col_order))

# 2) num states in each race/year
num_states_table <- accuracy_table %>%
  mutate(Year = as.numeric(Year)) %>%
  distinct(Year, Race, State) %>%
  group_by(Year, Race) %>%
  summarise(Number_of_States = n(), .groups = "drop") %>%
  pivot_wider(
    names_from  = Race,
    values_from = Number_of_States
  ) %>%
  rename(
    President             = President,
    `U.S. Senate`         = `US Senate`,
...
Modal category population estimates in CPS and CES
The output of this code chunk is an excel sheet called demographic_modal_summary.xlsx which serves as the input for the modal category population estimates table in the figuresHQ notebook.
```{r}
...
common_demo_vars <- c("SEX_rc", "EDUC_rc", "VETSTAT_rc", "EMPSTAT_rc", "UNION_rc", "REGION_rc", "AGE_GROUP_rc", "VOTED_rc", "VOTEHOW_rc", "CITIZEN_rc", "HISPAN_rc", "FAMINC_rc")


# Function to calculate weighted proportions
calculate_proportions <- function(df, var, weight_col = NULL) {
  if (is.null(weight_col)) {
    # Unweighted proportions
    props <- prop.table(table(df[[var]]))
  } else {
    # Weighted proportions
    props <- weighted.mean(df[[var]] == levels(df[[var]]), df[[weight_col]], na.rm = TRUE)
  }
  return(props * 100)  # Convert to percentages
}


process_year <- function(year_data, year) {
  #browser()  # Optional: Remove or comment out this line if not needed for debugging
  ces_df <- year_data$CES
  cps_df <- year_data$CPS
  ...
  for (demo_var in common_demo_vars) {
    print(paste("Processing variable:", demo_var))
    
    # Skip if variable doesn't exist in either dataset
    if (!(demo_var %in% names(ces_df)) || !(demo_var %in% names(cps_df))) {
      print(paste("Variable not found in datasets:", demo_var))
      next
    }
    
    # Get all categories for this variable
    categories <- unique(c(ces_df[[demo_var]], cps_df[[demo_var]]))
    categories <- categories[!is.na(categories)]
    categories <- sort(categories)  # Ensure categories are sorted for consistency
    
    #print(paste("Categories for variable", demo_var, ":", paste(categories, collapse = ", ")))
    
    # Calculate proportions for CPS
    cps_props <- prop.table(table(factor(cps_df[[demo_var]], levels = categories))) * 100
    #print(paste("CPS proportions calculated for variable", demo_var))
    
    # Calculate unweighted proportions for CES
    ces_unweighted_props <- prop.table(table(factor(ces_df[[demo_var]], levels = categories))) * 100
    #print(paste("CES unweighted proportions calculated for variable", demo_var))
    
    # Calculate weighted proportions for CES using wpct from the weights package
    ces_weighted1_props <- wpct(factor(ces_df[[demo_var]], levels = categories), weight = ces_df[[weight_vars[1]]]) * 100
    #print(paste("CES weighted 1 proportions calculated for variable", demo_var))
    
    ces_weighted2_props <- wpct(factor(ces_df[[demo_var]], levels = categories), weight = ces_df[[weight_vars[2]]]) * 100
    #print(paste("CES weighted 2 proportions calculated for variable", demo_var))
    ...
    demo_results <- data.frame(
      ...
      CES_Weighted1 = as.numeric(ces_weighted1_props),
      CES_Weighted2 = as.numeric(ces_weighted2_props)
    )
    
    all_results <- rbind(all_results, demo_results)
    #print(paste("Results combined for variable", demo_var))
  }
  
  return(all_results)
}

all_processed_years <- list()  # Initialize an empty list to store results

for (i in 1:9) {
  all_processed_years[[i]] <- process_year(list_all_years[[i]], list_all_years[[i]]$year)
}

# Combine the results into one data frame
full_demo_table <- do.call(rbind, all_processed_years)

# Rename the columns
full_demo_table <- full_demo_table %>%
  rename(
    ANESRake_Weighted = CES_Weighted2,
    CES_Weighted = CES_Weighted1
  )

# Add difference columns
full_demo_table <- full_demo_table %>%
  mutate(
    Difference_CPS_CES_Weighted = CPS - CES_Weighted,
    Difference_CPS_CES_Unweighted = CPS - CES_Unweighted,
    Difference_CPS_ANESRake_Weighted = CPS - ANESRake_Weighted
  )

# Find the modal category for each Variable across all years
# 1. Identify modal categories for each variable for each year
modal_category_summary <- full_demo_table %>%
  group_by(Year, Variable) %>%
  filter(CPS == max(CPS)) %>%
  ungroup()

# 2. Determine the most common modal category for each variable
most_common_modal_categories <- modal_category_summary %>%
  group_by(Variable, Category) %>%
  summarise(count = n()) %>%
  arrange(Variable, desc(count)) %>%
  group_by(Variable) %>%
  slice(1) %>%  # Take the most common category (there might be a tie)
  ungroup() %>%
  select(Variable, Category)

# 3. Join back to the full_demo_table to filter for these categories and take abs value
selected_modal_categories <- full_demo_table %>%
  semi_join(most_common_modal_categories, by = c("Variable", "Category")) %>%
  mutate(across(starts_with("Difference"), ~abs(.)))

# 4. Compute the averages for these fixed modal categories across all years
average_modal_summary <- selected_modal_categories %>%
  group_by(Variable, Category) %>%
  summarise(
    Average_CPS = round(mean(CPS), 1),
    Average_CES_Unweighted = round(mean(CES_Unweighted), 1),
    Average_CES_Weighted = round(mean(CES_Weighted), 1),
    Average_ANESRake_Weighted = round(mean(ANESRake_Weighted), 1),
    Average_Difference_CPS_CES_Unweighted = round(mean(Difference_CPS_CES_Unweighted), 1),
    Average_Difference_CPS_CES_Weighted = round(mean(Difference_CPS_CES_Weighted), 1),
    Average_Difference_CPS_ANESRake_Weighted = round(mean(Difference_CPS_ANESRake_Weighted), 1),
    .groups = 'drop'  # This suppresses the warning
  )

# export average_modal to excel
write.xlsx(average_modal_summary, "tables_and_figures/demographic_modal_summary.xlsx")
```
