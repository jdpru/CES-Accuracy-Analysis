...
write_xlsx(conflict_dfs$same_party_diff_votes, "same_party_diff_votes.xlsx")
write_xlsx(conflict_dfs$same_party_same_votes, "same_party_same_votes.xlsx")
write_xlsx(conflict_dfs$diff_party_same_votes, "diff_party_same_votes.xlsx")
...
```{r}
test1 <- CES_20_rc_clean %>%
  filter(POST_STATE_rc == "WEST VIRGINIA")

unique(test1$AG_CANDIDATE_rc)
unique(test1$AG_PARTY_rc)
table(test1$AG_PARTY_rc)

test <- CES_20_raw %>%
  filter(inputstate_post == 54) %>%
  select(CC20_414a, AttCand1Name, AttCand1Party, AttCand2Name, AttCand2Party, AttCand3Name, AttCand3Party)
```
