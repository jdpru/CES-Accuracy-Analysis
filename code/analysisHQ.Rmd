...
This will be used to prettify the accuracy tables at the end and split one big table into a list of yearly tables
```{r}
...
# Results
Now that accuracy_tables_pretty is created, we compute different accuracy metrics in different ranges for each major party.
```{r}
...
We wanted to check if race accuracies differ based on whether race was federal or state level 
```{r}
...
This overall_hist_table will serve as the input to the figuresHQ notebook used for creating figures.
```{r}
...
## TEMPORARY -- For Investigating high errors
```{r}
high_error <- CES_10_rc_clean %>%
  filter(POST_STATE_rc == "NEBRASKA") %>%
  select(V100, POST_STATE_rc, ST_SENATOR_rc)

high_error$CC413c <- CES_06_rc_clean$CC413c[CES_06_rc_clean$caseid == high_error$v1000]
```
...
The output of this code chunk is an excel sheet called demographic_modal_summary.xlsx which serves as the input for the modal category population estimates table in the figuresHQ jupyter notebook.
```{r}
