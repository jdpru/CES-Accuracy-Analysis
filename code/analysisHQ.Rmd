...
pckgs <- c("haven", "readxl", "ipumsr", "here", "dplyr", "cdlTools", "anesrake", "writexl", "tidyverse", "rlang", "tibble", "naniar", "weights", "lubridate", "openxlsx", "cpsR", "purrr", "progress", "tidycensus")
...
7/17 investigating 0s and NAs for total votes in master sheet State_race_master_table
```{r}
investigation <- State_race_master_table %>%
  filter(`Votes  candidate` == 0 | is.na(`Votes  candidate`))

# randomly select 15 rows from investigation
investigation_sample <- investigation %>%
  sample_n(15)

investigation_all_zero <- State_race_master_table %>%
  group_by(`State (full name)`, `Election date`, `Office/district`) %>%
  dplyr::summarize(all_zero_or_na = all(is.na(`Votes  candidate`) | `Votes  candidate` == 0), .groups = "drop") %>%
  filter(all_zero_or_na)

# View results
print(zero_or_na_races)
```
...
recode_cand_to_party <- function(list_all_years, yrly_ces_candidate_names, yrly_ces_house_candidate_names) {
  for (i in seq_along(list_all_years)) {
    ...
    if (as.numeric(year) < 2012) {
      cat("Skipping year (because does not require recode):", year, "\n")
      next
    }
...
Update list_all_years, run the recoding, then push changes from list_all_years back to global environment to keep list_all_years synced with each individual CES year in the global environment. Note: This will produce a message that 2006, 2008, and 2010 were skipped. This is by design since the CES changed how they reported candidate parties starting in 2012.
```{r}
update_list_all_years()
