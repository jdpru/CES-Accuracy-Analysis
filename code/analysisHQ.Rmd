...
ces_08_cand_name_file <- suppressWarnings(
  read_csv(
    "data/CES Data/2008 CCES Election File (House and Senate).csv",
    col_types = cols_only(
...
State_legislative_0623 <- read_excel("data/benchmarks/Election Returns/State_Legislative_and_Constitutional_Offices_2006_2023.xlsx", guess_max = 20000)

special_state <- State_legislative_0623 %>% 
  filter(special_election == TRUE,
         year(election_date) %% 2 == 0)

# remaining to check
remaining <- anti_join(
  special_state,
  rows_only_in_special_state,
  by = names(special_state)
)



# find rows that are NOT duplicates but are still special election == TRUE
rows_only_in_special_state <- anti_join(
  special_state,
  dupes_new,
  by = names(special_state)
) %>%
  filter(year(election_date) %% 2 == 0)
...
# this code HIGIHLIGHTS THE FACT THAT MANY CANDIDATES JHAVE DUPLICATED DATA, EVEN THOUGH THEY ONLY APPEARE ONCE IN OFFICIAL ELECTION RETUNRS. BALLOTPEDIA NEEDS TO BE NOTIFIED. AND THIS NEEDS TO BE REMIEDE. AND JON NEEDS TO BE EMAILED. 
```{r}
find_vote_conflict_scenarios <- function(df) {

  # Columns that define a "record" apart from vote counts
  id_cols <- intersect(
    c("state", "election_date", "office_district", "district", "candidate"),
    names(df)
  )

  party_col <- "party_simplified"

  if (!all(c("votes", party_col) %in% names(df))) {
    stop("Required columns missing (votes, party_simplified).")
  }

  # Identify duplicated records ignoring votes
  dupes <- df %>%
    group_by(across(all_of(id_cols))) %>%
    filter(n() > 1) %>%
    ungroup()

  # Compute group-level diagnostics
  classified <- dupes %>%
    group_by(across(all_of(id_cols))) %>%
    mutate(
      same_party = n_distinct(.data[[party_col]], na.rm = TRUE) == 1,
      same_votes = n_distinct(votes, na.rm = TRUE) == 1
    ) %>%
    ungroup() %>%
    arrange(state, election_date, office_district, district, candidate)

  list(
    same_party_diff_votes = classified %>%
      filter(same_party, !same_votes),

    same_party_same_votes = classified %>%
      filter(same_party, same_votes),

    diff_party_same_votes = classified %>%
      filter(!same_party, same_votes),

    diff_party_diff_votes = classified %>%
      filter(!same_party, !same_votes)
  )
}


df_new <- State_legislative_0623

conflict_dfs <- find_vote_conflict_scenarios(df_new)

same_party_diff_votes <- conflict_dfs$same_party_diff_votes
same_party_same_votes <- conflict_dfs$same_party_same_votes
diff_party_same_votes <- conflict_dfs$diff_party_same_votes
```
...
create_targets <- function(year_data_list) {
  ...
  for (year_data in year_data_list) {
    ...
    variables <- variables[!grepl("DUM|OVERTIME|REGION_rc|CUR_MIL_rc|CLASSWKR_rc|CITIZEN_rc", variables)]
...
add_weights_to_df <- function(df, year, weighting_results, col_name) {
  ...
  # Check if the weight column is already present
  if (col_name %in% colnames(df)) {
    message(paste0(col_name, " already present in ", year))
    return(df)
  }
  
  # Extract weights and case IDs for the specified year
...
```{r}
CES_06_rc_clean <- add_weights_to_df(CES_06_rc_clean, 2006, weighting_results, "anesrake_weight_full")
CES_08_rc_clean <- add_weights_to_df(CES_08_rc_clean, 2008, weighting_results, "anesrake_weight_full")
CES_10_rc_clean <- add_weights_to_df(CES_10_rc_clean, 2010, weighting_results, "anesrake_weight_full")
CES_12_rc_clean <- add_weights_to_df(CES_12_rc_clean, 2012, weighting_results, "anesrake_weight_full")
CES_14_rc_clean <- add_weights_to_df(CES_14_rc_clean, 2014, weighting_results, "anesrake_weight_full")
CES_16_rc_clean <- add_weights_to_df(CES_16_rc_clean, 2016, weighting_results, "anesrake_weight_full")
CES_18_rc_clean <- add_weights_to_df(CES_18_rc_clean, 2018, weighting_results, "anesrake_weight_full")
CES_20_rc_clean <- add_weights_to_df(CES_20_rc_clean, 2020, weighting_results, "anesrake_weight_full")
CES_22_rc_clean <- add_weights_to_df(CES_22_rc_clean, 2022, weighting_results, "anesrake_weight_full")
```
...
```{r}
# These are the variables that were NOT selected by anesrake to use in weighting
vars_used_vs_all <- do.call(
  rbind,
  lapply(names(weighting_results), function(y_chr) {

    # Keys might be numeric or character; normalize
    y <- as.character(y_chr)

    # Variables proposed to ANESRake for this year
    proposed <- names(all_targets[[y]])

    # Variables actually used by ANESRake
    used <- weighting_results[[y]]$vars_used

    # Build one row per proposed variable
    data.frame(
      Year     = y,
      Variable = proposed,
...
# Just the excluded variables (not used)
...
#   here("tables_and_figures/input_tables", "dist_errors_bucket_table.xlsx")
# )
# 
# # Race Competitiveness
# write_xlsx(
#   election_rmse_by_competitiveness_range,
#   here("tables_and_figures/input_tables", "election_rmse_by_competitiveness_range.xlsx")
# )
# 
# # Party + Candidate combined for comparison
# write_xlsx(
#   party_candidate_combined_valid,
#   here("tables_and_figures/input_tables", "party_candidate_combined_valid.xlsx")
# )
```
...
```{r}
banded <- most_specific %>%
  filter(
    Validity_Scheme == "ANESRake - Full",
  ...
  mutate(
    Comp_Band = case_when(
      ...
      Benchmark <= 60 ~ "55–60%",
      Benchmark <= 80 ~ "60–80%",
      Benchmark <= 100 ~ "80–100%",
      TRUE ~ NA_character_
    )
  )

election_rmse_by_competitiveness_range <- bind_rows(
  banded,
  banded %>% mutate(Comp_Band = "0–100%")
) %>%
  group_by(
    Year,
    Weighting_Method,
  ...
  summarise(
    ...
    Avg_Abs_Error = mean(abs(Error), na.rm = TRUE),
    n_observations = sum(!is.na(Error)),
    n_states = n_distinct(State),
    n_respondents = sum(n_respondents, na.rm = TRUE),
    .groups = "drop"
  )
...
NOTE THAT NOW, WHEN FILTERING, YOU MUST PICK A VALIDITY_scheme, THEN IF YOU DO A SPECIFICITY LEVEL + DEMOGRAPHICS, IT MUST BE SPECIFICITY LEVEL & IS.NA(SPECIFICITY)
########### re write above
...
## Distribution of errors buckets
```{r}
dist_errors_bucket_table <- metrics_base %>%
  mutate(
    Abs_Error = abs(Error)
  ) %>%
  group_by(Validity_Scheme) %>%
  mutate(
    Error_Bucket = cut(
      Abs_Error,
      breaks = seq(
        0,
        ceiling(max(Abs_Error, na.rm = TRUE)),
        by = 1
      ),
      right = FALSE,
      include.lowest = TRUE
  ...
  group_by(
    ...
    Weighting_Method,
    Error_Bucket,
    .drop = FALSE   # keep zero-count buckets
  ) %>%
  summarise(
    Count = n(),
    .groups = "drop"
...
## Candidate vs. Party-level specificity
```{r}
party_long_to_combine <- party_demovote_long %>%
  mutate(
    Specificity = "Party",
    District = NA_character_,
    CES_Candidate = NA_character_,
    True_Candidate = NA_character_,
    Match_Score = NA_real_
  ) %>%
  filter(Class == "Candidate Choice")

candidate_long_to_combine <- candidate_demovote_long %>%
  mutate(
    Specificity = "Candidate"
  ) %>%
  filter(Class == "Candidate Choice")

party_candidate_combined <- bind_rows(
  party_long_to_combine,
  candidate_long_to_combine
)

party_candidate_combined_valid <- bind_rows(

  # ALL
  party_candidate_combined %>%
    mutate(Validity_Scheme = "All"),
  ...
  select(
    Year, State, District, Class, Specificity, Validity_Scheme,
    Variable, Variable_Type, Category,
    Used_in_ANESRake_Full, Used_in_ANESRake_Restricted,
    Valid_for_Accuracy_Full, Valid_for_Accuracy_Restricted,
    CES_Candidate, True_Candidate, Match_Score,
    Benchmark, CES_Unweighted, CES_Weighted,
    CES_ANESRake_Full, CES_ANESRake_Restricted,
    n_respondents, Weighting_Method, Error
  )
```
...
<!--
# Create summary tables
calc sumary stats OLD
```{r}
# ==============================================================================
# SUMMARY TABLES FOR EXPLORATION AND ANALYSIS
# ==============================================================================
# These tables use ALL variables (no filtering) and include detailed sample size metrics

# RMSE by Year × Variable × Weighting Method (all variables)
rmse_summary_all <- most_specific_long %>%
...
# RMSE by Year × Class × Weighting Method (all variables)
rmse_by_class_all <- most_specific_long %>%
...
# RMSE by Year × Variable_Type × Weighting Method
rmse_by_variable_type_all <- most_specific_long %>%
...
# ==============================================================================
# FILTERED TABLE FOR ACCURACY ASSESSMENT
# ==============================================================================
# IMPORTANT: For unbiased accuracy assessment, filter using the appropriate
# Valid_for_Accuracy_* flag for the weighting scheme being evaluated.
#
# Valid_for_Accuracy_Full = TRUE means:
#   - Variable_Type == "Secondary" (not used in CES-provided weighting), AND
#   - Used_in_ANESRake_Full == FALSE (not used in full ANESRake weighting)
#
# Valid_for_Accuracy_Restricted = TRUE means:
#   - Variable_Type == "Secondary" (not used in CES-provided weighting), AND
#   - Used_in_ANESRake_Restricted == FALSE (not used in restricted ANESRake weighting)
most_specific_long_valid <- bind_rows(
  # ===============================
  # FULL validity scheme
  # ===============================
  most_specific_long %>%
    filter(Valid_for_Accuracy_Full) %>%
    filter(
    ...
    mutate(Validity_Scheme = "Full"),

  # ===============================
  # RESTRICTED validity scheme
  # ===============================
  most_specific_long %>%
    filter(Valid_for_Accuracy_Restricted) %>%
    filter(
    ...
    mutate(Validity_Scheme = "Restricted")
)


# Filtered RMSE by Year × Variable × Weighting Method
rmse_summary_valid <- most_specific_long_valid %>%
  group_by(
    Validity_Scheme,
    Year,
    Variable,
...
# Filtered RMSE by Year × Class × Weighting Method
rmse_by_class_valid <- most_specific_long_valid %>%
  group_by(
    Validity_Scheme,
    Year,
    Class,
...
# Filtered RMSE by Year × Variable_Type × Weighting Method
rmse_by_variable_type_valid <- most_specific_long_valid %>%
  group_by(
    Validity_Scheme,
    Year,
    Variable_Type,
...
```{r}
# ==============================================================================
# EXCEL EXPORT TABLES FOR PUBLICATION/VISUALIZATION
# ==============================================================================
# These tables are exported to Excel for use in figuresHQ.ipynb and publications
# All use Valid_for_Accuracy filter for unbiased accuracy assessment

# ---------------------------------------------------------------
# 1. Wide-format summary table: Years × Variables matrix
# ---------------------------------------------------------------

ces_wtd_rmse_summary_table <- most_specific_long %>%
  filter(Weighting_Method == "CES Weights",
  ...
  mutate(`Year Average` = rowMeans(across(where(is.numeric)), na.rm = TRUE)) %>%
  bind_rows(
    summarise(
      .,
...
# ---------------------------------------------------------------
# 3. Yearly RMSE by Weighting Method
# ---------------------------------------------------------------

rmse_by_weighting_method <- most_specific_long %>%
  filter(Valid_for_Accuracy == TRUE) %>%
  group_by(Year, Variable, Weighting_Method) %>%
  summarise(RMSE = sqrt(mean(Error^2, na.rm = TRUE)), .groups = "drop") %>%
...
# ---------------------------------------------------------------
# 4. Yearly RMSE by Weighting Method and Class
# ---------------------------------------------------------------
...
# Combine and save
rmse_by_weighting_method_and_class <- bind_rows(rmse_by_class, rmse_all) %>%
  arrange(Year, Weighting_Method, Class)

write_xlsx(
  rmse_by_weighting_method_and_class,
  here("tables_and_figures/input tables", "rmse_by_weighting_method_and_class.xlsx")
)

# ---------------------------------------------------------------
# 5. Candidate choice accuracy by race competitiveness
# ---------------------------------------------------------------
...
election_rmse_by_competitiveness_range <- most_specific_long %>%
  filter(Class == "Candidate Choice") %>%
...
calculate_turnout_comparison <- function(year_data, year) {
  ...
  # Get weight variables from config
  weight_vars <- config$weight_vars[[as.character(year)]]
...
## testing some shit
```{r}
# make a new version of most specific long that ingores voting registration status and rows where the discrepany between CES_Weighted and Benchmark is less than 10%

test1 <- most_specific_long %>%
  filter(abs(Error) > 30)

test2 <- most_specific_long %>%
  filter(abs(Error) > 30) %>%
  filter(Variable == "Voter Registration")

test3 <- most_specific_long %>%
  filter(Variable %notin% c("Voter Registration", "Voting Turnout")) %>%
  mutate(Discrepancy = abs(CES_Weighted - Benchmark)) %>%
  filter(Discrepancy > 30)

testdoc <- read_csv("/Users/jdpruett/Downloads/elections-2006.csv")
...
# ---------- 1) Load data ----------
# Master (single sheet)
master_path <- "data/Benchmarks/Election Returns/[FIN-FIX] Historical election results data (2006 - 2023) clean fixed.xlsx"
State_race_master_table_DUP <- read_excel(master_path) %>% 
  clean_names()

# AR/NY file (only the "Arkansas" tab)
arny_path <- "/Users/jdpruett/Desktop/PPRG/PPRG_R_Directory/AR_NY_Missing_Election_Results.xlsx"
ar_ny_raw <- read_excel(arny_path, sheet = "Arkansas") %>% 
  clean_names()

# ---------- 2) Minimal cleaning & alignment ----------
# Normalize election date to DATE (not datetime) so 04:00 vs 05:00 UTC doesn't break matches
normalize_dates <- function(df) {
  df %>% 
    mutate(
      election_date = suppressWarnings(ymd_hms(election_date, quiet = TRUE)),
      election_date = as.Date(election_date) # drop time-of-day
    )
}

master <- State_race_master_table_DUP %>% normalize_dates()
ar_ny  <- ar_ny_raw %>% normalize_dates()

# Optional: trim strings to avoid stray whitespace mismatches
trim_all_chars <- function(df) {
  df %>% mutate(across(where(is.character), ~ str_squish(.x)))
}
master <- trim_all_chars(master)
ar_ny  <- trim_all_chars(ar_ny)

coerce_key_types <- function(df) {
  df %>%
    mutate(
      # join on DATE already handled earlier
      office_number_for_sorting = as.character(office_number_for_sorting),  # <- was numeric in one, char in the other
      special_election = as.character(special_election)  # guard against logical vs character mismatches
    )
}

master <- master %>% coerce_key_types()
ar_ny  <- ar_ny  %>% coerce_key_types()

# --- Party normalization ---
normalize_party <- function(x) {
  x0 <- str_squish(str_to_lower(as.character(x)))
  x0 <- str_replace_all(x0, "[^a-z0-9 ]", " ")         # drop punctuation
  x0 <- str_squish(x0)
  # remove trailing "party"
  x0 <- str_replace(x0, "\\bparty\\b$", "")
  x0 <- str_squish(x0)

  dplyr::case_when(
    x0 %in% c("democratic", "democrat", "dem", "d", "dfl", "democratic farmer labor") ~ "Democratic",
    x0 %in% c("republican", "republic", "rep", "r", "gop")                            ~ "Republican",
    x0 %in% c("green", "g")                                                           ~ "Green",
    x0 %in% c("libertarian", "lib", "l")                                              ~ "Libertarian",
    x0 %in% c("constitution")                                                         ~ "Constitution",
    x0 %in% c("independent", "indy", "ind", "no party", "nonpartisan", "non partisan",
              "unaffiliated", "none")                                                 ~ "Independent/Nonpartisan",
    x0 %in% c("write in", "writein")                                                  ~ "Write-In",
    is.na(x0) | x0 == ""                                                              ~ NA_character_,
    TRUE                                                                              ~ str_to_title(x0) # fallback: title-case whatever remains
  )
}

master <- master %>% mutate(political_party_norm = normalize_party(political_party))
ar_ny  <- ar_ny  %>% mutate(political_party_norm = normalize_party(political_party))

library(stringr)
library(dplyr)

# -------- OFFICE NORMALIZATION HELPERS --------

# Extract a district number from free text like:
# "House District 12", "HD 12", "Dist. 12", "D12", "Senate 12", "SD-12", "Place 3" (optional)
extract_district <- function(txt, fallback_num = NA) {
  if (is.null(txt)) txt <- NA_character_
  s <- tolower(as.character(txt))

  # Try standard "district ##" variants first
  m <- str_match(s, "\\b(?:district|dist\\.?|d)\\s*-?\\s*(\\d{1,3})\\b")[,2]
  # If not found, try HD/SD forms
  if (all(is.na(m))) m <- str_match(s, "\\b(?:hd|sd)\\s*-?\\s*(\\d{1,3})\\b")[,2]
  # If still not found, try "house/senate ##"
  if (all(is.na(m))) m <- str_match(s, "\\b(?:house|senate)\\s*-?\\s*(\\d{1,3})\\b")[,2]
  # Optional: sometimes "Place 3" is used (comment out if not desired)
  # if (all(is.na(m))) m <- str_match(s, "\\bplace\\s*(\\d{1,3})\\b")[,2]

  # Fall back to provided numeric column if available
  if (all(is.na(m)) && !all(is.na(fallback_num))) {
    suppressWarnings({
      num <- as.numeric(fallback_num)
    })
    m <- ifelse(is.na(num), NA_character_, as.character(as.integer(num)))
  }

  # Standardize as integer-like character
  m <- ifelse(is.na(m), NA_character_, as.character(as.integer(m)))
  m
}

# Classify the office into a standardized category
classify_office <- function(office_raw) {
  s <- tolower(as.character(office_raw))
  s <- str_replace_all(s, "[^a-z0-9 ]", " ")
  s <- str_squish(s)

  case_when(
    str_detect(s, "\\b(house|assembly|representative|assemblymember|state rep\\.?|\\bsr\\b|\\bhd\\b|house ?district|stateRepresentative|HouseDistrict|partyresentative)\\b") ~ "State House/Assembly",
    str_detect(s, "\\b(senate|senator|\\bss\\b|\\bsd\\b|senate ?district|kysenator)\\b") ~ "State Senate",

    str_detect(s, "attorney") ~ "Attorney General",
    str_detect(s, "secretar|secratary") ~ "Secretary of State",
    str_detect(s, "\\btreas") ~ "Treasurer",
    str_detect(s, "auditor") ~ "Auditor",
    str_detect(s, "governor\\b") ~ "Governor",
    str_detect(s, "lieutenant|lt\\.?\\s*gov") ~ "Lieutenant Governor",
    str_detect(s, "comptroller") ~ "Comptroller",
    str_detect(s, "superintendent|public instruction|education") ~ "Superintendent of Public Instruction",
    str_detect(s, "insurance") ~ "Insurance Commissioner",
    str_detect(s, "agricultur|ag comm") ~ "Commissioner of Agriculture",
    str_detect(s, "public service|utilities? commission|railroad commission") ~ "Public Service Commission",

    TRUE ~ str_to_title(s) # fallback: keep a readable label
  )
}

# Normalize office fields on a data frame:
# - office_category_norm (standardized chamber/office)
# - district_norm (only for legislative seats; statewide -> NA)
normalize_office_fields <- function(df) {
  df %>%
    mutate(
      office_category_norm = classify_office(office_district),
      # Only try to extract districts for legislative chambers
      district_norm = if_else(
        office_category_norm %in% c("State House/Assembly", "State Senate"),
        extract_district(office_district, fallback_num = office_number_for_sorting),
        NA_character_
      )
    )
}
...
master <- master %>% normalize_office_fields()
ar_ny  <- ar_ny  %>% normalize_office_fields()


# ---------- 3) Define key columns (all non-vote fields) ----------
# After janitor::clean_names(), your columns should be like:
# state_full_name, election_date, office_district, office_number_for_sorting,
# candidate_name_copy_and_paste_from_source, political_party, votes_candidate,
# total_votes_in_race_if_included_in_the_source, source_url, special_election

# We'll match on everything EXCEPT the vote columns.
key_cols <- c(
  "state_full_name",
  "election_date",
  "office_category_norm",          # <-- normalized office category
  #"district_norm",                 # <-- normalized district (NA for statewide)
  #"candidate_name_copy_and_paste_from_source",
  "political_party_norm"      # from the earlier party-normalization step
)

# Safety check: ensure all keys exist; if not, stop with a clear message
missing_in_master <- setdiff(key_cols, names(master))
missing_in_arny   <- setdiff(key_cols, names(ar_ny))
if (length(missing_in_master) > 0) stop("Master missing key columns: ", paste(missing_in_master, collapse = ", "))
if (length(missing_in_arny) > 0)   stop("AR_NY missing key columns: ", paste(missing_in_arny, collapse = ", "))

# Coerce vote columns to numeric for clean comparisons
coerce_votes <- function(df){
  df %>% mutate(
    votes_candidate = suppressWarnings(as.numeric(votes_candidate)),
    total_votes_in_race_if_included_in_the_source = suppressWarnings(as.numeric(total_votes_in_race_if_included_in_the_source))
  )
}
master <- coerce_votes(master)
ar_ny  <- coerce_votes(ar_ny)

# ---------- 4) Build the three buckets ----------
# 1) Rows that match on keys BUT master is missing votes
bucket1_exact_match_missing_votes <- ar_ny %>%
  inner_join(
    master %>% select(all_of(key_cols), votes_candidate),
    by = key_cols,
    suffix = c("_arny", "_master")
  ) %>%
  filter(is.na(votes_candidate_master)) %>%            # <-- fixed
  select(all_of(key_cols),
         votes_candidate_arny,
         votes_candidate_master)

# 2) Rows in AR_NY that are NOT present in master (by key)
bucket2_not_in_master <- ar_ny %>%
  anti_join(master, by = key_cols)

# 3) Present in master, but master already has a value or it differs from AR_NY
bucket3_conflict_with_master <- ar_ny %>%
  inner_join(
    master %>% select(all_of(key_cols), votes_candidate),
    by = key_cols,
    suffix = c("_arny", "_master")
  ...
  filter(
    !is.na(votes_candidate_master) &                   # master has a value
      (is.na(votes_candidate_arny) |                   # AR_NY missing OR different
       votes_candidate_arny != votes_candidate_master)
  ) %>%
  select(all_of(key_cols),
         votes_candidate_arny,
         votes_candidate_master)

# ---------- 5) (Optional) Inspect counts ----------
cat("\nBucket 1 (match on keys, master missing votes): ", nrow(bucket1_exact_match_missing_votes))
cat("\nBucket 2 (not in master): ", nrow(bucket2_not_in_master))
cat("\nBucket 3 (present but conflicting/filled votes): ", nrow(bucket3_conflict_with_master), "\n")

# ---------- 6) (Optional) Write out to an xlsx with 3 tabs ----------
# Uncomment if you want a file with the results.
# library(writexl)
# writexl::write_xlsx(
#   list(
#     bucket1_exact_match_missing_votes = bucket1_exact_match_missing_votes,
#     bucket2_not_in_master             = bucket2_not_in_master,
#     bucket3_conflict_with_master      = bucket3_conflict_with_master
#   ),
#   path = "arkansas_vote_merge_buckets.xlsx"
# )


# Spot-check how things got categorized
count(master, office_category_norm, sort = TRUE)
count(ar_ny,  office_category_norm, sort = TRUE)

# Verify district extraction for a few examples
ar_ny %>% 
  select(office_district, office_number_for_sorting, office_category_norm, district_norm) %>% 
  head(20)

# See what changed in bucket2 after normalization
nrow(bucket2_not_in_master)
```
