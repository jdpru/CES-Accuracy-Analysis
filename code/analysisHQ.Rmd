...
vars_used_ces_weights <- list(
  ...
  `2020` = c("AGE_GROUP_rc", "EDUC_rc", "SEX_rc", "HISPANIC_rc", "VOTEREG_rc", "POST_STATE_rc", "GOV_CANDIDATE_rc", "GOV_PARTY_rc", "SENATOR_CANDIDATE_rc", "SENATOR_PARTY_rc"),
...
```{r}
grep("HOUSE_CAND", names(CES_06_raw), value = TRUE)
```
...
cand_keep <- cc_base %>%
  filter(
    ...
    (
      # 2010–2016: President, Senate, House, Governor
      (Year >= 2010 & Year <= 2016 &
         Variable %in% c("President", "U.S. Senate", "U.S. House", "Governor")) |

      # 2018–2022: add SecState + AG
      (Year >= 2018 & Year <= 2022 &
         Variable %in% c(
...
party_keep <- cc_base %>%
  filter(
    Specificity == "Party",
    (
      # 2006–2008: all candidate choice from party
      (Year %in% c(2006, 2008)) |
