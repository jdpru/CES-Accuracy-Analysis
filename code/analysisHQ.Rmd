...
setwd('/path/to/your/CES Accuracy Analysis')
...
pckgs <- c("haven", "readxl", "ipumsr", "here", "dplyr", "cdlTools", "anesrake", "writexl", "tidyverse", "rlang", "tibble", "naniar", "weights", "lubridate", "openxlsx", "cpsR", "purrr", "progress")
...
# Add STATE_rc to CPS data frames
# NOTE THIS CAN TAKE AWHILE (10 min) - the fips package is very slow
CPS_06_rc_clean$STATEFIP <- as.numeric(CPS_06_rc_clean$STATEFIP)
CPS_08_rc_clean$STATEFIP <- as.numeric(CPS_08_rc_clean$STATEFIP)
CPS_10_rc_clean$STATEFIP <- as.numeric(CPS_10_rc_clean$STATEFIP)
CPS_12_rc_clean$STATEFIP <- as.numeric(CPS_12_rc_clean$STATEFIP)
CPS_14_rc_clean$STATEFIP <- as.numeric(CPS_14_rc_clean$STATEFIP)
CPS_16_rc_clean$STATEFIP <- as.numeric(CPS_16_rc_clean$STATEFIP)
CPS_18_rc_clean$STATEFIP <- as.numeric(CPS_18_rc_clean$STATEFIP)
CPS_20_rc_clean$STATEFIP <- as.numeric(CPS_20_rc_clean$STATEFIP)
CPS_22_rc_clean$STATEFIP <- as.numeric(CPS_22_rc_clean$STATEFIP)

CPS_06_rc_clean$STATE_rc <- toupper(fips(CPS_06_rc_clean$STATEFIP, to="Name"))
CPS_08_rc_clean$STATE_rc <- toupper(fips(CPS_08_rc_clean$STATEFIP, to="Name"))
CPS_10_rc_clean$STATE_rc <- toupper(fips(CPS_10_rc_clean$STATEFIP, to="Name"))
CPS_12_rc_clean$STATE_rc <- toupper(fips(CPS_12_rc_clean$STATEFIP, to="Name"))
CPS_14_rc_clean$STATE_rc <- toupper(fips(CPS_14_rc_clean$STATEFIP, to="Name"))
CPS_16_rc_clean$STATE_rc <- toupper(fips(CPS_16_rc_clean$STATEFIP, to="Name"))
CPS_18_rc_clean$STATE_rc <- toupper(fips(CPS_18_rc_clean$STATEFIP, to="Name"))
CPS_20_rc_clean$STATE_rc <- toupper(fips(CPS_20_rc_clean$STATEFIP, to="Name"))
CPS_22_rc_clean$STATE_rc <- toupper(fips(CPS_22_rc_clean$STATEFIP, to="Name"))
...
# Compute Election Candidate Choice Errors
Compute election candidate choice errors in each state/race by comparing vote proportions for each major party in the CES to vote proportions in the true election returns
```{r}
...
## Define workhouse election error function
```{r}
create_election_error_table <- function(yearly_ces_wt_vars,
                                         election_results_df,
                                         year_data_list,
                                         NC_flag_df) {
  #browser()
...
```{r}
election_error_table <- create_election_error_table(
  yearly_ces_wt_vars,
...
election_error_tables_pretty <- make_tables_pretty(election_error_table)
```
...
calculate_demoplus_error <- function(year_data, year) {
  ...
  for (state in unique(na.omit(ces_df$POST_STATE_rc))) {
    ...
    cps_state <- filter(cps_df, STATE_rc == state)
...
Errors for candidate choice and demographic-plus variables have been computed and stored in separate tables. These tables now need to be filtered to retain only modal categories, reshaped to a long format, and combined into a unified table of all accuracy variables.

## Reshaping and modalizing candidate choice
```{r}
# Bind all years of candidate choice error into one big df
election_error_table <- bind_rows(election_error_tables_pretty)

# pivot_longer on the three error‐columns
election_error_table_long <- election_error_table %>%
  pivot_longer(
...
# 6/25 find out why this is different
test_modal_election_error_table_long <- election_error_table_long %>%
  group_by(Year, Race, State) %>%
  filter(Actual_Percent == max(Actual_Percent, na.rm = TRUE)) %>%
  ungroup()

modal_party_rows <- election_error_table_long %>%
  group_by(Year, State, Race) %>%
  slice_max(order_by = Actual_Percent, n = 1, with_ties = FALSE) %>%
  ungroup()

modal_election_error_table_long <- election_error_table_long %>%
...
## Combine candidate choice and demoplus tables
```{r}
modal_election_harmonized <- modal_election_error_table_long %>%
  mutate(
    Variable = Race,
    Category = CES_Party,
...
modal_error_table_all <- bind_rows(
  modal_election_harmonized,
...
# Results
Now that an error table has been created, subset that table to compute RMSE and report various other breakdowns of error. Outputs are excel sheets that serve as inputs to 
figuresHQ.ipynb
...
ces_wtd_rmse_summary_table <- modal_error_table_all %>%
...
write_xlsx(
  ...
  here("tables_and_figures", "ces_wtd_rmse_summary_table.xlsx")
)
...
```{r}
dist_errors_bucket_table <- modal_error_table_all %>%
  mutate(
    Abs_Error = abs(Value),
    Error_Bucket = cut(
      Abs_Error,
  ...
  group_by(Weighting_Method, Error_Bucket) %>%
  summarise(
    Count = n(),
    .groups = "drop"
  )
...
write_xlsx(
  ...
  here("tables_and_figures", "dist_errors_bucket_table.xlsx")
)
```

Yearly RMSE by Weighting Method
```{r}
rmse_by_weighting_method <- modal_error_table_all %>%
...
write_xlsx(
  ...
  here("tables_and_figures", "rmse_by_weighting_method.xlsx")
)
```
...
election_rmse_by_competitiveness_range <- modal_error_table_all %>%
...
write_xlsx(
  ...
  here("tables_and_figures", "election_rmse_by_competitiveness_range.xlsx")
)
```
